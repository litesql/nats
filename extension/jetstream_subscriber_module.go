...
func (m *JetStreamSubscriberModule) Connect(conn *sqlite.Conn, args []string, declare func(string) error) (sqlite.VirtualTable, error) {
	...
	if virtualTableName == "" {
		virtualTableName = config.DefaultJetStreamSubscriberVTabName
	}
	...
	if tableName == "" {
		tableName = config.DefaultJetStreamTableName
	}
	...
	vtab, err := NewJetStreamSubscriberVirtualTable(virtualTableName, servers, opts, timeout, tableName, conn, logger)
	if err != nil {
		return nil, err
	}
	return vtab, declare("CREATE TABLE x(stream TEXT, subject TEXT, durable TEXT)")
}
