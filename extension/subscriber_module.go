...
func (m *SubscriberModule) Connect(conn *sqlite.Conn, args []string, declare func(string) error) (sqlite.VirtualTable, error) {
	...
	vtab, err := NewSubscriberVirtualTable(virtualTableName, servers, opts, tableName, conn, logger)
	if err != nil {
		return nil, err
	}
	return vtab, declare("CREATE TABLE x(subject TEXT PRIMARY KEY, queue_group TEXT)")
}
